...
## R Markdown vs Quarto：该用哪个？

| 要素 | 说明 |
|------|------|
| **文件** | `06_R_Markdown报告模板.Rmd` 和 `06_Quarto报告模板.qmd` |
| **核心理念** | Quarto 是 R Markdown 的下一代继任者，功能更强大 |

仓库中同时提供了两种格式的模板，方便大家了解两者的差异：

| 对比项 | R Markdown (.Rmd) | Quarto (.qmd) |
|--------|-------------------|---------------|
| 状态 | 成熟稳定，大量存量项目 | 积极开发中，未来方向 |
| 代码折叠 | 需要额外配置 | 原生支持 `code-fold` |
| 输出格式 | PDF / HTML / Word | PDF / HTML / Word / 演示 / 书籍 / 网站 |
| 多语言 | 主要支持 R | R + Python + Julia + Observable JS |
| Callout 块 | 不支持 | 原生支持 (note/warning/tip/important) |
| 推荐场景 | 维护旧项目 | **新项目首选** |

> **建议**：新项目直接用 Quarto，旧项目 R Markdown 继续用即可，无需迁移。
> 两者的核心理念一样：代码 + 文本 + 可视化 = 一份可复现的报告。
...
**关于 Rust 篇的定位说明**：Rust 是一门系统编程语言，学习曲线较陡，对于大部分业务同事来说并非必须掌握。本部分的核心价值不在于让大家学会 Rust，而在于展示 **AI 如何帮你理解一个全新领域的复杂概念**。所有权的生活类比教学本身就是一个很好的"AI 辅助学习"示范——即使你以后不写 Rust，这种"用已知理解未知"的思维方式同样适用于学习任何新技能。如果你是技术人员或对系统编程感兴趣，Rust 篇的内容会更有深度。
...
| — | `prompt_examples.md` | — | 总览 | AI 提示词实战手册 |
| — | `ai_tool_guide.md` | — | 总览 | AI 工具选择指南 |
| 01 | `01_pandas入门_生活例子.py` | Python | 入门 | pandas 筛选 + 生活类比 |
| 02 | `02_debug练习_找bug.py` | Python | 练习 | 5 类 Bug 调试训练（含提示） |
| 02 | `02_debug练习_找bug_纯净版.py` | Python | 练习 | 不含提示的练习版 |
| 02 | `02_debug练习_找bug_答案.py` | Python | 练习 | 教师参考答案 |
...
| 06 | `06_R_Markdown报告模板.Rmd` | R | 高级 | R Markdown PDF 报告 |
| 06 | `06_Quarto报告模板.qmd` | R | 高级 | Quarto 新一代报告模板 |
...
| 10 | `10_业务实战_月度分析报告.py` | Python | 实战 | AI 辅助完成业务分析报告 |
...
python 02_debug练习_找bug_纯净版.py      # 练习 - 调试 Bug（先试纯净版）
pip install flask && python 03_flask应用_图书管理.py  # 进阶 - Flask API
python 10_业务实战_月度分析报告.py        # 实战 - 业务分析报告
