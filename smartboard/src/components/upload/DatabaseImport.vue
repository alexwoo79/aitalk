<template>
  <div class="import-card" :class="{ 'has-tables': tables.length > 0 }">
    ...
    <div class="db-mode-toggle">
      <button class="mode-btn" :class="{ active: dbMode === 'sqlite' }" @click="dbMode = 'sqlite'">SQLite {{
        t('upload.dbFile') }}</button>
      <button class="mode-btn" :class="{ active: dbMode === 'remote' }" @click="dbMode = 'remote'">MySQL / PG</button>
    </div>
    <template v-if="dbMode === 'sqlite'">
      <button class="btn btn-sm card-btn" @click="selectFile" :disabled="loading">{{ selectedFile ? t('common.edit') +
        '...' : t('upload.selectFile') }}</button>
    </template>
    <template v-if="dbMode === 'remote'">
      <input v-model="connString" type="text" class="card-input" :placeholder="t('upload.dbConnPlaceholder')"
        @keyup.enter="testConnection" />
      <div class="card-btn-row">
        <button class="btn btn-sm card-btn" @click="testConnection" :disabled="loading || !connString">{{ loading ?
          t('upload.feishuTesting') : t('upload.dbTestConn') }}</button>
    ...
    <div v-if="tables.length > 0" class="tables-section">
      ...
      <div class="tables-list">
        <label v-for="table in tables" :key="table.name" class="table-row"
          :class="{ selected: selectedTables.includes(table.name) }">
          <input type="checkbox" :value="table.name" v-model="selectedTables" />
          <span class="table-name">{{ table.name }}</span>
          <span v-if="isSqlite && table.row_count !== undefined" class="table-meta">{{ table.row_count }} {{
            t('common.rows') }}</span>
        </label>
      </div>
      ...
      <div class="card-actions" v-if="selectedTables.length > 0">
        <button class="btn btn-primary" @click="importSelected" :disabled="loading">{{ loading ? t('upload.parsing') :
          t('upload.dbExecute') }}</button>
        <button v-if="loading" class="btn btn-sm btn-stop" @click="cancel">{{ t('common.stop') }}</button>
      </div>
    </div>
    <div class="sql-section" v-if="(isSqlite && selectedFile) || (!isSqlite && connString)">
      <div class="sql-header" @click="showSql = !showSql"><span>{{ t('upload.dbCustomSql') }}</span><span
          class="sql-arrow">{{ showSql ? '▲' : '▼' }}</span></div>
      <div v-if="showSql" class="sql-body">
        <textarea v-model="customSql" class="sql-textarea" :placeholder="t('upload.dbSqlPlaceholder')"
          rows="3"></textarea>
        <button class="btn btn-sm btn-primary" @click="executeSql" :disabled="loading || !customSql.trim()">{{ loading ?
          t('upload.parsing') : t('upload.dbExecute') }}</button>
      </div>
    </div>
