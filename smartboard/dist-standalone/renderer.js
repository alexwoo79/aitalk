var SmartboardRenderer = (function (exports) {
  ...
  function parseFilter(filter) {
    ...
    if (containsM) {
      const result = {
        column: containsM[1].trim(),
        op: "contains",
        value: containsM[3].trim(),
      };
      console.log(
        "[filter] parseFilter CONTAINS:",
        JSON.stringify(s),
        "→",
        JSON.stringify(result),
      );
  ...
  function applyFilter(rows, filter, conditions) {
    ...
    for (const group of andGroups) {
      ...
      result = result.filter((r) =>
        parsedOrConds.some((parsed) => matchRow$1(r, parsed)),
      );
    }
    return result;
  }
  ...
  function fmtByChart(n, chart, metricName) {
    ...
    const decimals =
      mf?.decimals !== void 0
        ? mf.decimals
        : chart.decimals !== void 0
          ? chart.decimals
          : 2;
    if (!fmtType || fmtType === "number" || fmtType === "global")
    ...
    const features = {
      ...
      dataView: {
        ...
        optionToContent: function (opt) {
          ...
          const thStyle =
            "padding:6px 10px;border-bottom:2px solid " + bd + ";color:" + txt;
          const tdStyle =
            "padding:4px 10px;border-bottom:1px solid " + bd + ";color:" + txt;
          const btnStyle =
            "margin-top:8px;padding:4px 12px;border:1px solid " +
            bd +
          ...
          if (isPie) {
            ...
            let html2 =
              '<div style="max-height:400px;overflow:auto;font-size:12px;color:' +
              txt +
              ";background:" +
              bg +
            ...
            html2 += "</tbody></table>";
            html2 +=
              `<button onclick="window._copyTable('` +
              tsv2.replace(/'/g, "\\'") +
              `',this)" style="` +
              btnStyle +
              '">' +
          ...
          if (isScatter) {
            ...
            let html2 =
              ...
              '"><table style="width:100%;border-collapse:collapse">';
            html2 +=
              '<thead><tr><th style="' + thStyle + ';text-align:left">#</th>';
            html2 +=
              '<th style="' + thStyle + ';text-align:right">' + xName + "</th>";
            html2 +=
              '<th style="' + thStyle + ';text-align:right">' + yName + "</th>";
            html2 +=
              '<th style="' +
              thStyle +
              ';text-align:left">' +
          ...
          let tsv =
            L.dimension + "	" + series.map((s) => s.name || "").join("	") + "\n";
          let html =
            '<div style="max-height:400px;overflow:auto;font-size:12px;color:' +
            txt +
            ";background:" +
          ...
          for (let i = 0; i < catData.length; i++) {
            html += "<tr>";
            html +=
              '<td style="' + tdStyle + '">' + (catData[i] || "") + "</td>";
            const row = [String(catData[i] || "")];
            series.forEach((s) => {
              const val = tf(s.data?.[i] ?? "");
              row.push(val);
              html +=
                '<td style="' + tdStyle + ';text-align:right">' + val + "</td>";
            });
            tsv += row.join("	") + "\n";
            html += "</tr>";
          }
      ...
      return {
        ...
        label: showLabel
          ? {
              ...
              position: "top",
              fontSize: 11,
              formatter: (p) => fmtByChart(p.value, chart, p.seriesName),
            }
          : void 0,
        markPoint: showLabel
          ? {
      ...
      tooltip: {
        ...
        formatter: (params) => {
          ...
          return params
            .map(
              (p) =>
                `${p.seriesName}: ${fmtByChart(p.value, chart, p.seriesName)}`,
            )
            .join("<br/>");
        },
      },
  ...
  function buildHorizontalBarOption(
    ...
    const series = metricCols.map((m, mi) => {
      ...
      return {
        ...
        itemStyle: {
          borderRadius:
            mi === metricCols.length - 1 ? [0, 4, 4, 0] : [0, 0, 0, 0],
          color: COLORS[mi % COLORS.length],
        },
      };
    });
  ...
  function buildDoughnutOption(chart, rows, showLabel = true) {
    ...
    return {
      ...
      tooltip: {
        ...
        textStyle: { color: "#eee" },
        formatter: (p) =>
          `${p.name}: ${fmtByChart(p.value, chart, metricCol)} (${p.percent}%)`,
      },
      legend: {
        bottom: 0,
      ...
      series: [
        {
          ...
          label: showLabel
            ? { show: true, fontSize: 10, formatter: "{b}\n{d}%" }
            : { show: false },
  ...
  function buildHistogramOption(chart, rows, binCount) {
    ...
    const projectedHeight = Math.ceil(
      maxLabelPx * Math.sin((rotateAngle * Math.PI) / 180),
    );
  ...
  function buildLineOption(chart, rows, areaFill = true, smooth = true) {
    ...
    const series = metricCols.map((mc, mi) => ({
      ...
      areaStyle: areaFill
        ? { color: COLORS[mi % COLORS.length] + "22" }
        : void 0,
    }));
    return {
      _mf: chart.metricFormats || {},
  ...
  function computeTimeseries(rows, dateCol, metricCol, period = "month") {
    ...
    const forecastLabels = nextPeriodKeys(
      labels[labels.length - 1],
      period,
      forecastCount,
    );
    const forecastValues = forecastLabels.map(
    ...
    return {
      ...
      yoy,
      trend,
      forecast: { labels: forecastLabels, values: forecastValues },
    };
  }
  ...
  function computeDeciles(rows, metricCol) {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      ranges.push(
        `${fmtCompact(bucket[0])}–${fmtCompact(bucket[bucket.length - 1])}`,
      );
    }
  ...
  function computeClusters(rows, metricCols, k = 3, xCol, yCol) {
    ...
    for (let iter = 0; iter < 20; iter++) {
      ...
      centroids = sums.map((s, i) =>
        s.count > 0 ? { x: s.x / s.count, y: s.y / s.count } : centroids[i],
      );
    }
    return {
  ...
  function applyConditionFilter(rows, expr) {
    ...
    const colMap = {};
    if (rows.length)
      for (const k of Object.keys(rows[0])) colMap[k.toLowerCase()] = k;
    for (const group of groups) {
      ...
      if (orConds.length)
        rows = rows.filter((r) => orConds.some((p) => matchRow(r, p)));
    }
    return rows;
  }
  function getFilteredRows() {
    ...
    for (const col of Object.keys(filterValues)) {
      ...
      if (v && v !== "__all__")
        rows = rows.filter((r) => String(r[col] ?? "").trim() === v);
    }
  ...
  function renderFilterBar() {
    ...
    cf.type = "text";
    cf.placeholder =
      t("dashboard.conditionPlaceholder") ||
      "e.g. Amount > 100 & Region = Beijing";
    cf.style.cssText =
      "padding:6px 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;min-width:200px";
    cf.oninput = () => {
      condFilter = cf.value;
      refreshAll();
    };
    ...
    si.placeholder = "搜索...";
    si.style.cssText =
      "padding:6px 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;width:120px";
    si.oninput = () => {
      searchText = si.value;
      refreshAll();
    };
  ...
  function renderDateRangeBar() {
    ...
    presets.forEach((p) => {
      ...
      btn.onclick = () => {
        ...
        si.value = dateStart;
        ei.value = dateEnd;
        pw.querySelectorAll(".dr-preset").forEach((b) =>
          b.classList.remove("active"),
        );
        btn.classList.add("active");
        refreshAll();
      };
      if (dateStart === p.start && dateEnd === p.end)
        btn.classList.add("active");
      pw.appendChild(btn);
    });
    bar.appendChild(pw);
  ...
  function buildDatePresets(dr) {
    ...
    if (totalMonths >= 3) {
      ...
      presets.push({
        label: t("dashboard.datePresets.last3Months"),
    ...
    if (totalMonths >= 6) {
      ...
      presets.push({
        label: t("dashboard.datePresets.last6Months"),
        start: s.toISOString().slice(0, 10),
        end: dr.max,
      });
    }
    ...
    for (let y = 1; y <= totalYears; y++) {
      ...
      presets.push({
        label:
          y === 1
            ? t("dashboard.datePresets.last1Year")
            : t("dashboard.datePresets.lastNYears", { n: String(y) }),
        start: s.toISOString().slice(0, 10),
        end: dr.max,
      });
    }
    presets.push({
      label: t("dashboard.datePresets.all"),
      start: dr.min,
      end: dr.max,
    });
    return presets;
  ...
  function updateDrInfo() {
    ...
    info.textContent =
      inRange + " / " + DATA.rows.length + " " + t("common.records");
  }
  ...
  function buildTsOption(td, chart) {
    ...
    const aD = [
      ...td.values,
      ...new Array(td.forecast.labels.length).fill(null),
    ];
    ...
    const fD = [
      ...new Array(td.labels.length - 1).fill(null),
      td.values[td.values.length - 1],
      ...td.forecast.values,
    ];
    return {
      tooltip: {
        ...
        formatter: (params) => {
          ...
          return params
            ...
            .map(
              (x) =>
                `${x.seriesName}: ${fmtByChart(x.value, chart, x.seriesName)}`,
            )
            .join("<br/>");
        },
      },
      ...
      series: [
        ...
        },
        {
          ...
          type: "line",
          data: mD,
          lineStyle: { color: COLORS[2], width: 2, type: "dashed" },
          itemStyle: { color: COLORS[2] },
          smooth: true,
          symbol: "none",
        },
        {
          ...
          type: "line",
          data: tD,
          lineStyle: { color: "#6B7280", width: 1.5, type: "dotted" },
      ...
      yAxis: [
        {
          ...
          axisLabel: { fontSize: 10, formatter: (v) => fmtCompact$1(v) },
        },
        {
          type: "value",
          position: "right",
  ...
  function renderCharts(rows) {
    ...
    DATA.chartSpecs.forEach((ch, i) => {
      try {
        ...
        const ms0 =
          ch.metrics && ch.metrics.length > 0
            ? ch.metrics
            : ch.metric
              ? [ch.metric]
              : [];
        ...
        if (ch.type === "timeseries") {
          ...
          if (ms.length > 1) {
            ...
            ms.forEach((m) => {
              ...
              b.textContent = m;
              b.onclick = () => {
                mb.querySelectorAll(".metric-btn").forEach((bb) =>
          ...
          [
            ...
            b.onclick = () => {
              pt.querySelectorAll(".period-btn").forEach((bb) =>
                bb.classList.remove("active"),
              );
              b.classList.add("active");
          ...
          const cm =
            ch.clusterMetrics && ch.clusterMetrics.length > 0
              ? ch.clusterMetrics
              : ch.metrics || [];
          if (cm.length > 2) {
          ...
          if (bm.length > 1) {
            ...
            if (ch.type === "doughnut" || ch.type === "histogram") {
              ...
              ms.onchange = () => {
                ...
                let opt2 = null;
                if (ch.type === "doughnut")
                  opt2 = buildDoughnutOption({ ...ch, metric: mc }, cRows);
              ...
              bm.forEach((m) => {
                ...
                b.onclick = () => {
                  ...
                  activeBms.length = 0;
                  bmb
                    .querySelectorAll(".metric-btn.active")
                    .forEach((ab) => activeBms.push(ab.textContent));
                  h3.textContent = resolveTitle(ch.title, activeBms);
                  let opt2 = null;
                  if (ch.type === "bar")
                    opt2 = buildBarOption(
                      { ...ch, metrics: activeBms.slice() },
                      cRows,
                      sortOrder,
                      showLabel,
                    );
                  else if (ch.type === "horizontal_bar")
                    opt2 = buildHorizontalBarOption(
                      { ...ch, metrics: activeBms.slice() },
                      cRows,
                      sortOrder,
                      showLabel,
          ...
          if (ch.type === "bar" || ch.type === "horizontal_bar") {
            ...
            lb.textContent = "标签";
            lb.style.cssText =
              "padding:4px 12px;border-radius:16px;border:1px solid #e2e8f0;background:#3B82F6;color:#fff;font-size:12px;cursor:pointer";
            lb.onclick = () => {
              showLabel = !showLabel;
          ...
          let opt = null;
          if (ch.type === "bar")
            opt = buildBarOption({ ...ch, metrics: bm }, cRows, "none", true);
          else if (ch.type === "horizontal_bar")
            opt = buildHorizontalBarOption(
              { ...ch, metrics: bm },
              cRows,
              "none",
              true,
            ...
            opt = buildHistogramOption(ch, cRows);
  ...
  function renderTsChart(card, ch, rows, mc, pd) {
    ...
    if (!td) {
      wrap.textContent = t("chart.insufficientData", {
        name: t("chart.timeseries"),
      });
      return;
    }
    ...
    const lastM = td.mom[td.mom.length - 1],
      lastY = td.yoy[td.yoy.length - 1];
    info.innerHTML =
      `<span>最新: <strong>${td.labels[td.labels.length - 1]}</strong></span><span>值: <strong>${fmt(td.values[td.values.length - 1])}</strong></span>` +
      (lastM != null
        ? `<span>环比: <strong style="color:${lastM >= 0 ? "#10B981" : "#EF4444"}">${lastM >= 0 ? "+" : ""}${lastM.toFixed(1)}%</strong></span>`
        : "") +
      (lastY != null
        ? `<span>同比: <strong style="color:${lastY >= 0 ? "#10B981" : "#EF4444"}">${lastY >= 0 ? "+" : ""}${lastY.toFixed(1)}%</strong></span>`
        : "") +
      (td.forecast.values.length > 0
        ? `<span>下期预测: <strong>${fmt(td.forecast.values[0])}</strong></span>`
        : "");
  ...
  function renderTsDetailTable(card, td) {
    ...
    let html =
      ...
      t("chart.detailTable.period") +
      "</th><th>" +
      t("chart.detailTable.actualValue") +
      '</th><th title="' +
      t("chart.detailTable.ma3Title") +
      '">MA3</th><th>' +
      t("chart.detailTable.mom") +
      "</th><th>" +
      t("chart.detailTable.yoy") +
      "</th><th>" +
      t("chart.detailTable.trend") +
      "</th><th>" +
      t("chart.detailTable.forecast") +
      "</th></tr></thead><tbody>";
    td.labels.forEach((l, j) => {
      const mv = td.mom[j],
        yv = td.yoy[j];
      const mc = mv != null ? (mv >= 0 ? "#10B981" : "#EF4444") : "",
        yc = yv != null ? (yv >= 0 ? "#10B981" : "#EF4444") : "";
  ...
  function renderDecileChartContent(card, ch, rows, mc) {
    ...
    let html =
      '<table class="detail-table"><thead><tr><th>' +
      t("chart.detailTable.group") +
      "</th><th>" +
      t("chart.detailTable.count") +
      "</th><th>" +
      t("chart.detailTable.sum") +
      "</th><th>" +
      t("chart.detailTable.avg") +
      "</th><th>" +
      t("chart.detailTable.range") +
      "</th></tr></thead><tbody>";
  ...
  function removeOldElements(card) {
    const els = card.querySelectorAll(
      ".ts-info,.ts-detail-wrap,.detail-toggle,.cluster-summary",
    );
  ...
  function renderTable(rows) {
    ...
    tbs.style.cssText =
      "padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;width:120px";
    tbs.oninput = () => {
      tblSearch = tbs.value;
    ...
    tbc.type = "text";
    tbc.placeholder =
      t("dashboard.conditionPlaceholderShort") || "Amount > 100";
    tbc.style.cssText =
      "padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;width:160px";
    tbc.oninput = () => {
      tblCond = tbc.value;
      renderTableContent(rows, el, tb);
    };
    ...
    tbn.type = "number";
    tbn.value = String(tblRowLimit === "all" ? "" : tblRowLimit);
    tbn.min = "5";
    tbn.max = "500";
    tbn.style.cssText =
      "padding:4px 8px;border:1px solid #e2e8f0;border-radius:6px;font-size:12px;width:60px";
    tbn.onchange = () => {
      tblRowLimit =
        tbn.value === "" || tbn.value === "all"
          ? "all"
          : parseInt(tbn.value) || 15;
      renderTableContent(rows, el, tb);
    };
    tb.appendChild(tbn);
    ...
    DATA.tableColumns.forEach((c) => {
      ...
      l.style.cssText =
        "padding:2px 8px;border-radius:4px;font-size:11px;border:1px solid #e2e8f0;cursor:pointer;user-select:none;" +
        (tblCols.includes(c)
  ...
  function renderTableContent(rows, el, tb) {
    ...
    if (sortCol) {
      sorted.sort((a, b) => {
        const va = getNumericVal$1(a[sortCol]),
          vb = getNumericVal$1(b[sortCol]);
    ...
    h3.textContent = `${t("dashboard.dataTable")} · ${sl.length} / ${filtered.length} ${t("common.rows")}`;
        ...
        if (v == null || v === "") v = "—";
          ...
          if (cl?.type === "numeric" && cl?.role === "metric") {
            ...
            if (!isNaN(n)) {
              ...
              if (md && md.format && md.format !== "global")
                v = fmtByChart(n, { metricFormats: DATA.metricDefaults }, c);
              else v = fmt(n);
            }
          }
        }
  ...
  function refreshAll() {
    ...
    if (fc)
      fc.textContent = `${t("common.currentFilter")}: ${rows.length} ${t("common.records")}`;
    updateDrInfo();
  }
